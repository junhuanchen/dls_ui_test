...
class CST816:
    ...
    def _reg_write(self, reg, value):
        ...
        while True:
            try:
                return self.i2c.writeto(_CST816_ADDR, bytes([reg, value]))
            except OSError as e:
                #print(e)
                from fpioa_manager import fm
                from Maix import GPIO
                tmp = fm.fpioa.get_Pin_num(fm.fpioa.I2C1_SDA)
                fm.register(tmp, fm.fpioa.GPIOHS20)
                sda = GPIO(GPIO.GPIOHS20, GPIO.OUT)
                sda.value(1)
...
# 2. I2C 总线
i2c = I2C(I2C.I2C1, freq=400000, scl=30, gscl=fm.fpioa.GPIOHS19, sda=31, gsda=fm.fpioa.GPIOHS20)
