...
class Robot:
    ...
    def event_express(self, player, event):
        if event.data["action"] == "Face" or event.data["action"] == "shake": # 区分走路和运动。
            ...
            self.social.add(2) # 强烈摇晃 或 看到人，社交值拉爆
        # 生理需求处理
        if event.data["action"] == "battry_down" or event.data["action"] == "battry_up":
            self.life.set(event.data["value"]) # 电量事件，直接设置电量
        # 测试表情表达处理
        if event.data["action"] == "emoji":
            player.start(directory=self.get_path(event.data["value"]), loop=int(event.data["loop"]))